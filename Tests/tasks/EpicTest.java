...
class EpicTest {
    ...
    public void shouldBeNewEpicWhenAllSubtaskNew() {
        epic.setSubtask(new Subtask(epic.getIndex(), "NameSubtask1", "DesSubtask1", 1));
        epic.setSubtask(new Subtask(epic.getIndex(), "NameSubtask2", "DesSubtask2", 2));
        assertEquals(StatusTask.NEW, epic.getStatus());
    }

    @Test
    public void shouldBeDoneEpicWhenAllSubtaskDone() {
        Subtask firstSubtask = new Subtask(epic.getIndex(), "NameSubtask1", "DesSubtask1", 1);
        firstSubtask.setStatus(StatusTask.DONE);
        epic.setSubtask(firstSubtask);
        Subtask secondSubtask = new Subtask(epic.getIndex(), "NameSubtask2", "DesSubtask2", 2);
    ...
    public void shouldBeHave60MinutesDurationEpicWithOneSubtask() {
        epic.setSubtask(new Subtask(epic.getIndex(), "name1", "des1", 2, 60,
                "06.05.2022 05:00"));
    ...
    public void shouldBeHave60MinutesDurationEpicWithThreeSubtasksAndOneStartTime() {
        epic.setSubtask(new Subtask(epic.getIndex(), "name1", "des1", 2, 20,
                "06.05.2022 05:00"));
        epic.setSubtask(new Subtask(epic.getIndex(), "name2", "des2", 3, 20,
                "06.05.2022 05:00"));
        epic.setSubtask(new Subtask(epic.getIndex(), "name3", "des3", 4, 20,
                "06.05.2022 05:00"));
        assertEquals("06.05.2022 06:00", epic.getEndTime().format(epic.getFormatter()));
    }
